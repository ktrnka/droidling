...
DataSet = namedtuple('DataSet',
                     ['data', 'row_labels', 'targets', 'feature_names', 'target_names', 'regression_targets'])
...
def pcaExperiments(dataset):
    ...
    for color, target_value, target_name in zip("rg", [0, 1], dataset.target_names):
        pl.scatter(X_r[dataset.targets == target_value, 0], X_r[dataset.targets == target_value, 1], c=color,
                   label=target_name)
    pl.legend()
...
def generalGraph(dataset):
    ...
    pl.figure()
    for color, target_value, target_name in zip("rg", [0, 1], dataset.target_names):
        pl.scatter(dataset.data[dataset.targets == target_value, dataset.feature_names.index('Combined Score')],
                   dataset.data[dataset.targets == target_value, dataset.feature_names.index('Frequency')], c=color,
                   label=target_name)
    pl.legend()
...
def regressionExperiments(dataset, topN=5):
    ...
    for i, (train, test) in enumerate(folds):
        print "Fold {0}\n\tTraining examples: {1}\n\tTesting examples: {2}".format(i,
                                                                                   ", ".join([str(j) for j in train]),
                                                                                   ", ".join([str(j) for j in test]))
